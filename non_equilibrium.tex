...
is the parameter. The form of this model is justified empirically, although a formal argument based on a diagonalisation of the evolution semi-group can be made, and using the conjugate symmetry property
...
\begin{equation}
    ...
    \widehat{\rho}^{\mathrm{GK}}_{F,N_{\mathrm{iter}}}=\beta\int_0^\infty C_{\theta^*}(t)\,\dif t,
\end{equation}
...
\[\widehat{\rho}^{\mathrm{GK}}_{F,N_{\mathrm{iter}}}=\beta\sum_{k=1}^m\frac{a_k\lambda_k}{\lambda_k^2+f_k^2}.\]
\subsection{Mobility}
...
In Figure \ref{fig:fit_gk_mobility}, we show the fitted non-linear least squares model \eqref{eq:time_correlation_parametric_model} for the autocorrelation function, using $m=4$ modes.
...
%%autocorr fit:0.121(5)
%%autocorr int:0.121(8)
%% time : 1.628 \times 10^5
\subsection{Shear viscosity}